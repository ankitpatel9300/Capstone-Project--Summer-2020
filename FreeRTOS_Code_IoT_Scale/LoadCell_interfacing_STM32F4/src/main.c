...
const TickType_t xTickstoDelay= 500 ;
...
void vMQ2_Sensor_Task(void *pvParametrs){
	...
	while(1){
		vTaskDelay(xTickstoDelay) ;
		mq2sensor_flag= GPIO_ReadInputDataBit(GPIOA,GPIO_PinSource0); // Read MQ-2 Sensor Pin status flag
		if(mq2sensor_flag)
		{
			USART_SendString(USART2,"Gas detected\r\n" ); // if flag is set send notification to LTE module
		}
		vTaskDelay(xTickstoDelay) ;
	}
}
void vHX711_Task(void *pvParameters) {
	...
	myLoadCell.PD_SCK_PinNumber = GPIO_Pin_6;  // Set GPIOA Pin 6 as SCK
	myLoadCell.DOUT_PinType = GPIOA;
	myLoadCell.DOUT_PinNumber = GPIO_Pin_7;    // Set GPIOA Pin 7 as DOUT
	myLoadCell.gain = 0;
	...
	while (1) {
		vTaskDelay(xTickstoDelay) ;
		weight= HX711_GetValue(&myLoadCell);   // read weight
		char value[11]="weigh " ;
		itoa(weight,value+5,10);
		strcat(value,"gm");
		USART_SendString(USART2,weight); // Send reading to LTE module
	}
}
